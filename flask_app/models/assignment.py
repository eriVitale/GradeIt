...
class Assignment:
    ...
    def get_from_id(cls,data):
        query="SELECT * FROM assignments WHERE id=%(assignment_id)s;"
        results=connectToMySQL(cls.db).query_db(query,data)
        one_assignment=cls(results[0])
        one_assignment.grades=grade.Grade.get_grade_by_assignment({'assignment_id': results[0]['id']})
        return one_assignment

    @classmethod
    def save_grades(cls,data):
        query="INSERT INTO grades (student_id,assignment_id,grade) VALUES (%(student_id)s, %(assignment_id)s,%(grade)s);"
        return connectToMySQL(cls.db).query_db(query,data)

    @classmethod
    def update_grades(cls,data):
        query="UPDATE grades SET grade=%(grade)s WHERE student_id=%(student_id)s and assignment_id=%(assignment_id)s;"
        return connectToMySQL(cls.db).query_db(query,data)

    @classmethod
    def delete(cls,data):
        query="DELETE FROM assignments WHERE id=%(id)s;"
        return connectToMySQL(cls.db).query_db(query,data)
